...
import { Cog, Plus, X, AlertTriangle, Loader2, Bot } from 'lucide-react';
...
const DriveForm: React.FC<DriveFormProps> = ({ onSubmit, onCancel, initialData }) => {
  const {
    formData,
    setFormData,
    loading,
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      onSubmit({...formData, category: 'Drive'});
    }
  };
    ...
    <div className="bg-white rounded-lg shadow-md p-6 relative">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          <Cog className="text-green-500 mr-2" size={24} />
          <h2 className="text-xl font-semibold text-gray-900">Drive Component</h2>
        </div>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Torque</label>
                  <input
                    type="text"
                    value={formData.specifications?.torque || ''}
                    onChange={(e) => updateSpecification('torque', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                    placeholder="e.g., 25kg/cm"
                  />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Voltage</label>
                  <input
                    type="text"
                    value={formData.specifications?.voltage || ''}
                    onChange={(e) => updateSpecification('voltage', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                    placeholder="e.g., 12V"
                  />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Rotation Speed</label>
                  <input
                    type="text"
                    value={formData.specifications?.rotationSpeed || ''}
                    onChange={(e) => updateSpecification('rotationSpeed', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                    placeholder="e.g., 40 x 20 x 40 mm"
                  />
                </div>
              </div>
            </div>
